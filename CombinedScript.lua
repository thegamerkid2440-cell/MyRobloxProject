-- CombinedScript.lua
-- One self-contained Roblox Studio Luau source file.
--
-- Roblox still requires server and client execution contexts. Use this same
-- file as a normal Script in ServerScriptService and as a normal LocalScript
-- in StarterPlayerScripts (or another client container). The file detects its
-- context and runs the matching section.
--
-- Uses only standard Roblox APIs. No exploit APIs, loadstring, or external code.
...
if RunService:IsServer() then
	...
	local function applyTallAvatar(player: Player, character: Model?): boolean
		...
		-- Preserve clothing and accessories; change only R15 proportions.
	...
	local function applyOnRespawn(player: Player, character: Model)
		task.defer(function()
			...
			-- Wait for the character appearance to finish loading.
			task.wait(0.25)
	...
	local function watchPlayer(player: Player)
		player.CharacterAdded:Connect(function(character)
			if player:GetAttribute(TALL_AVATAR_ATTRIBUTE) == true then
				applyOnRespawn(player, character)
			end
		end)
	end
	...
	tallAvatarRemote.OnServerEvent:Connect(function(player: Player)
		-- The server performs the change. The client only sends a request.
...
elseif RunService:IsClient() then
	...
	if player then
		...
		-- Remove only a previous copy created by this file.
		local previousGui = playerGui:FindFirstChild("CombinedTallAvatarGui")
		if previousGui then
			previousGui:Destroy()
		end
